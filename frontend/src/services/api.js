...
export const getAdminUsers = async (page = 1, pageSize = 20, q = '', role = '') => {
  try {
    ...
    const response = await apiClient.get(`/api/users/admin/list?${params.toString()}`);
    return {
      success: true,
      data: response.data,
    };
...
export const deleteUserAdmin = async (userId) => {
  try {
    const response = await apiClient.delete(`/api/users/admin/${userId}`);
    return {
      success: true,
      data: response.data,
    };
...
export const updateUserAdmin = async (userId, data) => {
  try {
    const response = await apiClient.put(`/api/users/admin/${userId}`, data);
    return {
      success: true,
      data: response.data,
    };
